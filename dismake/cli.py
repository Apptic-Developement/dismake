...
def init_command(args: argparse.Namespace) -> Any:
    path = Path(".") / config_file
    try:
        Config.get_config(path)
    except:
        var: str = Prompt.ask(
            "[bold][cyan]?[/cyan][/bold] [bold]What is the name of your bot variable[/bold]",
            default="app",
        )
        console.clear()
        Config.create_config(path, var=var)
    else:
        return console.print(f"A {config_file!r} file already exists.", style="bold")
...
def run_command(args: argparse.Namespace) -> Any:
    ...
    try:
        ...
        exc_command = [
            "uvicorn",
            f"{config.dismake.main_file_name}:{config.dismake.bot.var}",
        ]
...
def vercel_command(args: argparse.Namespace) -> None:
...
def add_subparsers(parser: argparse.ArgumentParser) -> Any:
    ...
    init = subparsers.add_parser("init", help="Initialize a new config.toml file.")
    init.set_defaults(func=init_command)

    run = subparsers.add_parser("run", help="Run your bot.")
